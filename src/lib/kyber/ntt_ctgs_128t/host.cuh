//
// host.cuh
// Launches kernels NTT (128 threads version).
//
...
#ifndef ATPQC_CUDA_LIB_KYBER_NTT_CTGS_128T_HOST_CUH_
#define ATPQC_CUDA_LIB_KYBER_NTT_CTGS_128T_HOST_CUH_
...
namespace atpqc_cuda::kyber::ntt_ctgs_128t::host {
...
class fwdntt {
 ...
 public:
  ...
  explicit fwdntt(unsigned ninputs, unsigned poly_per_block = 1)
      : dg(make_uint3((k * ninputs + poly_per_block - 1) / poly_per_block, 1,
                      1)),
        db(make_uint3(128, poly_per_block, 1)),
        ns(sizeof(std::int16_t) * params::n * poly_per_block),
        np(k * ninputs) {}
...
class invntt_tomont {
 ...
 public:
  ...
  explicit invntt_tomont(unsigned ninputs, unsigned poly_per_block = 1)
      : dg(make_uint3((k * ninputs + poly_per_block - 1) / poly_per_block, 1,
                      1)),
        db(make_uint3(128, poly_per_block, 1)),
        ns(sizeof(std::int16_t) * params::n * poly_per_block),
